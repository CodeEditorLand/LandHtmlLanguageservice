	...
	doHover(
			...
			for (const provider of dataProviders) {
				...
				provider.provideTags().forEach((tag) => {
					if (tag.name.toLowerCase() === currTag.toLowerCase()) {
						let markupContent = generateDocumentation(
							tag,
							options,
							doesSupportMarkdown,
						);
		...
		function getAttrValueHover(
			...
			for (const provider of dataProviders) {
				...
				provider
					.provideValues(currTag, currAttr)
					.forEach((attrValue) => {
						if (
		...
		if (node.endTagStart && offset >= node.endTagStart) {
			const tagRange = getTagNameRange(
				TokenType.EndTag,
				node.endTagStart,
			);

	...
	return readFile(contentPath).then(function (content) {
		return getVersion(moduleName).then(function (version) {
			...
			try {
				...
				if (version) {
					console.log(
						"Updated " + path.basename(dest) + " (" + version + ")",
					);
				} else {
...
update(
	"js-beautify",
	"js/lib/beautify-html.js",
	"./src/beautify/esm/beautify-html.js",
	true,
	function (contents) {
		...
		if (index1 === -1) {
			throw new Error(
				`Problem patching beautify.html for ESM: '${topLevelFunction}' not found.`,
			);
		}
		if (index2 === -1) {
			throw new Error(
				`Problem patching beautify.html for ESM: '${outputVar}' not found after '${topLevelFunction}'.`,
			);
		}
		if (index3 === -1) {
			throw new Error(
				`Problem patching beautify.html for ESM: '${footer}' not found after '${outputVar}'.`,
			);
		}
		return (
			contents.substring(0, index1) +
